...
module ZEngine (MVar : MVar) = struct
  ...
  module ProtocolEngine = struct
    ...
    let send_nodes peer _nodes = 
      ...
      List.iter (fun node -> 
          ...
          let sdata = Message.with_marker               
              (StreamData(StreamData.create (true, true) 0L 0L None (MIOBuf.from_bytes (Lwt_bytes.of_bytes b))))
    ...
    let create ?(bufn = 32) ?(buflen=65536) (pid : MIOBuf.t) (lease : Vle.t) (ls : Locators.t) (peers : Locator.t list) strength (tx_connector: tx_session_connector) = 
      Guard.create @@ { 
        pid; 
        lease; 
        locators = ls; 
        ...
        qmap = QIDMap.empty;
        peers;
        router = ZRouter.create send_nodes (MIOBuf.hexdump pid) strength 2 0;
        next_mapping = 0L; 
        tx_connector;
        buffer_pool = Lwt_pool.create bufn (fun () -> Lwt.return @@ MIOBuf.create buflen) }
