...
let pid_to_string = MIOBuf.hexdump
...
let process_broker_open engine tsex msg = 
  ...
  let pe' = {pe with router = ZRouter.new_node pe.router {pid = MIOBuf.hexdump @@ Message.Open.pid msg; tsex}} in
  forward_all_decls pe;
...
let process_accept_broker engine tsex msg = 
  ...
  let pe' = {pe with router = ZRouter.new_node pe.router {pid = MIOBuf.hexdump @@ Message.Accept.apid msg; tsex}} in
  forward_all_decls pe;
  Guard.return [] pe'
