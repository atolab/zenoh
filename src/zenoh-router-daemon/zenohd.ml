...
let run tcpport peers strength plugins bufn style_renderer level = 
  ...
  let run () =  
    ...
    let res = Zengine.run tcpport peers strength bufn (Some (instream, outpush)) in
...
let () =
  ...
  let _ = Term.(eval (const run $ Zengine.tcpport $ Zengine.peers $ Zengine.strength $ Zengine.plugins $ Zengine.bufn $ Fmt_cli.style_renderer () $ Logs_cli.level ~env (), Term.info "zenohd")) in  ()
