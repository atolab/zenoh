...
let listener spath src datas = 
  ...
  let storage = List.fold_left (
    fun storage data -> (data |> fst |> Apero.decode_string |> Printf.printf "PLUGIN : STORAGE LISTENER [%-8s] RECIEVED RESOURCE [%-20s] : %s\n%!" spath src);
...
let qhandler spath resname predicate = 
  Printf.printf "PLUGIN : STORAGE QHANDLER [%-8s] RECIEVED QUERY : %s?%s\n%!" spath resname predicate;
  Lwt_mvar.take storagevar >>= fun storage -> 
...
let _ = 
  ...
  Zenoh.zopen "" 
  >>= fun z -> Zenoh.store z "/**" (listener "/**") (qhandler "/**") 
