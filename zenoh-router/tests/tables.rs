...
fn clean_test() {
    task::block_on(async{
        ...
        let primitives = Arc::new(Mux::new(Arc::new(DummyHandler::new())));
        let sex0 = Tables::declare_session(&tables, primitives.clone()).await;
        ...
        Tables::declare_subscription(&tables, &sex0, 0, "/todrop1/todrop11").await;
        let optres2 = Resource::get_resource(&tables.read()._get_root(), "/todrop1/todrop11");
        assert!(optres2.is_some());
        let res2 = optres2.unwrap();
        assert!(res2.upgrade().is_some());

        Tables::declare_subscription(&tables, &sex0, 1, "/todrop12").await;
        ...
        Tables::declare_subscription(&tables, &sex0, 0, "/todrop3").await;
        ...
        Tables::declare_subscription(&tables, &sex0, 0, "/todrop5").await;
        Tables::declare_subscription(&tables, &sex0, 0, "/todrop6").await;
...
fn client_test() {
    task::block_on(async{
        ...
        Tables::declare_subscription(&tables, &sex0, 11, "/**").await;
        Tables::declare_resource(&tables, &sex0, 12, 11, "/z1_pub1").await;

        let sex1 = Tables::declare_session(&tables, primitives.clone()).await;
        Tables::declare_resource(&tables, &sex1, 21, 0, "/test/client").await;
        Tables::declare_subscription(&tables, &sex1, 21, "/**").await;
        Tables::declare_resource(&tables, &sex1, 22, 21, "/z2_pub1").await;

        let sex2 = Tables::declare_session(&tables, primitives.clone()).await;
        Tables::declare_resource(&tables, &sex2, 31, 0, "/test/client").await;
        Tables::declare_subscription(&tables, &sex2, 31, "/**").await;
