...
use zenoh_router::routing::tables::{Tables, TablesHdl};
...
fn base_test() {
    task::block_on(async{
        ...
        declare_subscription(&mut *tables.write().await, &mut sex.upgrade().unwrap(), 1, "/four/five", &sub_info).await;
...
fn clean_test() {
    task::block_on(async{
        ...
        declare_subscription(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 0, "/todrop1/todrop11", &sub_info).await;
        let optres2 = Resource::get_resource(&tables.read().await._get_root(), "/todrop1/todrop11").map(|res| {Arc::downgrade(&res)});
        assert!(optres2.is_some());
        let res2 = optres2.unwrap();
        assert!(res2.upgrade().is_some());

        declare_subscription(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 1, "/todrop12", &sub_info).await;
        let optres3 = Resource::get_resource(&tables.read().await._get_root(), "/todrop1/todrop12").map(|res| {Arc::downgrade(&res)});
        assert!(optres3.is_some());
        let res3 = optres3.unwrap();
        assert!(res3.upgrade().is_some());

        undeclare_subscription(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 1, "/todrop12").await;
        assert!(res1.upgrade().is_some());
        assert!(res2.upgrade().is_some());
        assert!( ! res3.upgrade().is_some());

        undeclare_subscription(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 0, "/todrop1/todrop11").await;
        assert!(res1.upgrade().is_some());
        assert!( ! res2.upgrade().is_some());
        assert!( ! res3.upgrade().is_some());
        ...
        declare_subscription(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 0, "/todrop3", &sub_info).await;
        let optres1 = Resource::get_resource(&tables.read().await._get_root(), "/todrop3").map(|res| {Arc::downgrade(&res)});
        assert!(optres1.is_some());
        let res1 = optres1.unwrap();
        assert!(res1.upgrade().is_some());

        undeclare_subscription(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 0, "/todrop3").await;
        assert!(res1.upgrade().is_some());
        ...
        declare_subscription(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 0, "/todrop5", &sub_info).await;
        declare_subscription(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 0, "/todrop6", &sub_info).await;
...
fn client_test() {
    task::block_on(async{
        ...
        declare_subscription(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 11, "/**", &sub_info).await;
        ...
        declare_subscription(&mut *tables.write().await, &mut sex1.upgrade().unwrap(), 21, "/**", &sub_info).await;
        ...
        declare_subscription(&mut *tables.write().await, &mut sex2.upgrade().unwrap(), 31, "/**", &sub_info).await;

        primitives0.clear_data();
        primitives1.clear_data();
        primitives2.clear_data();
        route_data(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 0, "/test/client/z1_wr1", true, &None, RBuf::new()).await;
        ...
        primitives1.clear_data();
        primitives2.clear_data();
        route_data(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 11, "/z1_wr2", true, &None, RBuf::new()).await;
        ...
        primitives1.clear_data();
        primitives2.clear_data();
        route_data(&mut *tables.write().await, &mut sex1.upgrade().unwrap(), 0, "/test/client/**", true, &None, RBuf::new()).await; 
        ...
        primitives1.clear_data();
        primitives2.clear_data();
        route_data(&mut *tables.write().await, &mut sex0.upgrade().unwrap(), 12, "", true, &None, RBuf::new()).await; 
        ...
        primitives1.clear_data();
        primitives2.clear_data();
        route_data(&mut *tables.write().await, &mut sex1.upgrade().unwrap(), 22, "", true, &None, RBuf::new()).await; 
